...
export function createCategoryKey(name: string, type: TransactionTypeValue) {
  const key = name
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
...
export function createCategoryGroupKey(name: string, type: TransactionTypeValue) {
  const key = name
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
