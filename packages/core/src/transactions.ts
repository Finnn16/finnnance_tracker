...
export const transferFeeOptions = {
  BI_FAST: { label: "BI-FAST", amount: 2_500 },
  INTERNET: { label: "Internet", amount: 6_500 },
} as const;

export type TransferFeeMethod = keyof typeof transferFeeOptions;
...
export function getTransactionTypeLabel(type: TransactionTypeValue) {
  return (
...
function getDefaultDescription(type: TransactionTypeValue) {
...
export function validateTransactionPayload(
  ...
  if (!transactionTypes.has(type as TransactionTypeValue)) {
    return { ok: false, error: "Invalid transaction type." };
  }

  const transactionType = type as TransactionTypeValue;
    ...
    data: {
      ...
      transferFeeDescription:
        ...
        transferFeeMethod
          ...
          : null,
      transferFeeBudgetCategoryId: null,
