import { WalletType, type WalletType as WalletTypeValue } from "./domain-enums";
import { parseIntegerAmount } from "./money";

export const walletTypeOptions: Array<{ value: WalletTypeValue; label: string }> = [
...
export function getWalletTypeLabel(type: WalletTypeValue) {
  return (
...
export function validateWalletPayload(
  ...
  if (!walletTypeValues.has(type as WalletTypeValue)) {
